...
export const OptimizedNotepadApp: React.FC = React.memo(() => {
  ...
  const { getPerformanceSummary, isPerformanceDegrading } =
    usePerformanceMonitor("NotepadApp");
  ...
  // Confirmation dialog
  const { dialogState, showConfirmDialog, handleConfirm, handleCancel } =
    useConfirmDialog();
  ...
  const handleMathClick = useCallback(
    (latex: string, position: DOMRect, isInline: boolean) => {
