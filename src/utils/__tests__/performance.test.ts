...
describe("Performance utilities", () => {
  ...
  describe("measureExecutionTime", () => {
    ...
    it("handles errors and still measures time", async () => {
      ...
      await expect(
        measureExecutionTime("error test", () => {
  ...
  describe("performanceFeatures", () => {
    it("detects browser performance features", () => {
      expect(typeof performanceFeatures.supportsPerformanceObserver).toBe(
        "boolean"
      );
      expect(typeof performanceFeatures.supportsMemoryAPI).toBe("boolean");
      expect(typeof performanceFeatures.supportsIntersectionObserver).toBe(
        "boolean"
      );
      expect(typeof performanceFeatures.supportsRequestIdleCallback).toBe(
        "boolean"
      );
    });
  });
  ...
  describe("scheduleIdleWork", () => {
    ...
    it("falls back to setTimeout when requestIdleCallback is not available", () => {
      const mockSetTimeout = vi.spyOn(globalThis as any, "setTimeout");
      const originalRequestIdleCallback = (globalThis as any)
        .requestIdleCallback;
